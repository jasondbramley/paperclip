...
// The bundled skill docs live at the repo root (skills/, .agents/skills/). Resolve
// them relative to this file rather than process.cwd() so the assertions hold under
// both the repo-root wrapper (scripts/run-vitest-stable.mjs) and a raw
// `pnpm --filter @paperclipai/server exec vitest` run whose cwd is server/.
// __dirname here is server/src/__tests__, so the repo root is three levels up.
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
const fromRepoRoot = (relativePath: string) => path.resolve(REPO_ROOT, relativePath);
